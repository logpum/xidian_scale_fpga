...
always@(posedge clk) begin
	...
	r_lcd_dv <= (hcnt >= `H_SYNC + `H_BACK  && hcnt < `H_SYNC + `H_BACK + `H_DISP) &&
			(vcnt >= `V_SYNC + `V_BACK + `VSKIP_TOP  && vcnt < `V_SYNC + `V_BACK + `VSKIP_TOP + `VDISP_EN) 
			? 1'b1 : 1'b0;
	
    r_lcd_rgb 	<= 	lcd_data;
end 
assign lcd_rgb = r_lcd_dv ? r_lcd_rgb : 0; 
...
//ahead x clock
 ...
 always@(posedge clk) begin
    lcd_request	<=	(hcnt >= `H_SYNC + `H_BACK - H_AHEAD && hcnt < `H_SYNC + `H_BACK + `H_DISP_REQ - H_AHEAD) &&
						 (vcnt >= `V_SYNC + `V_BACK + `VSKIP_TOP && vcnt < `V_SYNC + `V_BACK + `VSKIP_TOP + `VDISP_EN) 
						 ? 1'b1 : 1'b0;

                         
// lcd xpos & ypos
    lcd_xpos	<= 	(hcnt - (`H_SYNC + `H_BACK - H_AHEAD));	//	lcd_request ? (hcnt - (`H_SYNC + `H_BACK - H_AHEAD)) : 11'd0;
    lcd_ypos	<= 	(vcnt - (`V_SYNC + `V_BACK)); 		//	lcd_request ? (vcnt - (`V_SYNC + `V_BACK)) : 12'd0;
end
